...
public class PlaylistsPanel extends BasicPage {
    ...
    public class InternalLineUp extends LineUpObject {
        ...
        protected Matcher<View> getMatcherToCountMaxIndexOfWindow() {
            return allOf(
                    withId( R.id.linear_playlist_layout ),
        ...
        public InternalCard card( int index ) {
            ...
            card.cover( label + " > Cover",
                    () -> allOf(
                            withId( R.id.linear_playlist_image ),
            ...
            card.title( label + " > Title",
                    () -> allOf(
                            withId( R.id.linear_playlist_title ),
                            isDescendantOfA( UtaPassUtil.withIndex(
                                    this.getMatcherToCountMaxIndexOfWindow(),
                                    indexInWindow ) ) ) ) ;

            card.likeCount( label + " > SubTitle",
                    () -> allOf(
                            withId( R.id.linear_playlist_like_count ),
                            isDescendantOfA( UtaPassUtil.withIndex(
                                    this.getMatcherToCountMaxIndexOfWindow(),
                                    indexInWindow ) ) ) ) ;
